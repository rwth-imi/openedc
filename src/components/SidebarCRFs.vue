<template lang="html">
  ...
  <b-nav vertical  class="mainPage">
    <div v-for="(crfItem, i) in crfs" :key="'crf_'+i">
      <b-nav-item v-bind:class="[crf.name == crfItem.name ? 'active' : '']">{{ crfItem.name }}</b-nav-item>
      <b-nav-item v-for="(sectionItem, index) in crfItem.sections" :key="sectionItem.name" v-bind:class="[section == index ? 'active' : '', 'subitem']">{{ sectionItem.name }}</b-nav-item>
    </div>
    <b-nav-item v-for="crfItem in crfs" :key="crfItem.name" v-bind:class="[crf.name == crfItem.name ? 'active' : '']">{{ crfItem.name }}</b-nav-item>
  </b-nav>
</div>
</template>
              ...
              subsection: [{
                ...
                items: [{
                  ...
                  },
                  {
                    ...
                    validationMin: null,
                    validationMax: null,
                    validationErrorMessage: 'Please enter the time in the 24 hour format (HH:MM)',
                    choices: null, // or null (or function)
                    branchingLogic: null, // or null
                    required: null,
                    defaultValue: null,
